...
def build_queue_b(participation: pd.DataFrame, hits: pd.DataFrame, core: pd.DataFrame,
                   notes: list[str]) -> pd.DataFrame:
    """Second stock per Core industry, from a DIFFERENT cap band than the Core pick:
    largest of Mid; if Mid is empty, largest of Small (per spec).

    Primary pool = the 15 Core industries with the most qualifying members, per spec.
    If that doesn't yield 15 usable picks (too few members, or no non-Large alternate
    band available), we extend down the full ranked list of the other 15 Core
    industries — same rule, just more candidates — so a real weekly run still lands
    on 15 whenever the underlying data can support it. Every extension is logged.
    """
    if core.empty:
        return pd.DataFrame(columns=["Ticker", "Industry", "Sector", "Quadrant", "Scan Tag", "Cap Band",
                                      "Contrasts With"])
    ...
    def try_fill(pool_df: pd.DataFrame, extended: bool):
        nonlocal skipped_top15
        for _, prow in pool_df.iterrows():
            if len(rows) >= 15:
                return
            industry = prow["Industry"]
            if any(r["Industry"] == industry for r in rows):
                continue
            ...
            if prow["Hits"] < 2:
                if not extended:
                    skipped_top15 += 1
                continue
            # Mid first, then Small (per spec); Large only as a last resort.
            pool = hits[(hits.Industry == industry) & (~hits.Symbol.isin(used))
                        & (hits.CapBand != core_pick["Cap Band"])]
            rep = None
            for band in ["Mid", "Small", "Large"]:
                band_pool = pool[pool.CapBand == band]
                if not band_pool.empty:
            ...
            used.add(rep["Symbol"])
            rows.append({"Ticker": rep["Symbol"], "Industry": industry, "Sector": rep["Sector"],
                         "Quadrant": core_pick["Quadrant"], "Scan Tag": rep["ScanTag"],
                         "Cap Band": rep["CapBand"], "Contrasts With": core_pick["Ticker"]})

    top15 = core_part.head(15)
    try_fill(top15, extended=False)

    if len(rows) < 15:
        rest = core_part.iloc[15:]
        before = len(rows)
        try_fill(rest, extended=True)
        if len(rows) > before:
            notes.append(f"Queue B: {skipped_top15} of the top-15-by-members industries had no usable "
                          f"second stock — filled {len(rows) - before} more slot(s) from the remaining "
                          f"Core industries, ranked the same way.")

    if len(rows) < 15:
        notes.append(f"Queue B: only {len(rows)}/15 slots filled — not enough Core industries had a "
                      f"second qualifying member in a different cap band.")
    return pd.DataFrame(rows)
...
def build_queue_c(participation: pd.DataFrame, hits: pd.DataFrame, core: pd.DataFrame, queue_b: pd.DataFrame,
                   notes: list[str]) -> pd.DataFrame:
    """Stock from the family the Core pick did NOT come from, for Core industries
    that qualified in both TREND and TURN, in an unused cap band. Per spec, ranked
    by participation rate, top 15.

    If that strict pool runs short — either because a candidate's only remaining
    stock sits in a cap band already used by Core/Queue B, or because fewer than
    15 Core industries have hits in both families at all — we relax in two logged
    stages so a real weekly run can still reach 15: first allow reusing a cap band
    (still a different stock, still the other family), then fall back to a second
    stock from the SAME family for Core industries that only qualified in one.
    """
    if core.empty:
        return pd.DataFrame(columns=["Ticker", "Industry", "Sector", "Quadrant", "Scan Tag", "Cap Band",
                                      "Contrasts With"])
    ...
    def used_bands_for(industry: str, core_pick: pd.Series) -> set[str]:
        bands = {core_pick["Cap Band"]}
        if not queue_b.empty:
            b_row = queue_b[queue_b.Industry == industry]
            if not b_row.empty:
                bands.add(b_row.iloc[0]["Cap Band"])
        return bands

    def pick(pool: pd.DataFrame) -> Optional[pd.Series]:
        for band in CAP_BANDS:
            band_pool = pool[pool.CapBand == band]
            if not band_pool.empty:
                return band_pool.sort_values("MarketCap", ascending=False).iloc[0]
        return pool.sort_values("MarketCap", ascending=False).iloc[0] if not pool.empty else None

    # Tier 1 (strict, per spec): both-family Core industries, unused cap band, other family.
    both = core_part[(core_part.TrendHits > 0) & (core_part.TurnHits > 0)].sort_values("Rate", ascending=False)
    reused_band_count = 0
    for _, prow in both.iterrows():
        if len(rows) >= 15:
            break
        industry = prow["Industry"]
        core_pick = core[core.Industry == industry].iloc[0]
        other_family = "TURN" if core_pick["Section"] == "TREND" else "TREND"
        avoid_bands = used_bands_for(industry, core_pick)
        pool = hits[(hits.Industry == industry) & (hits.Family == other_family) & (~hits.Symbol.isin(used))]
        rep = pick(pool[~pool.CapBand.isin(avoid_bands)])
        if rep is None:
            # Tier 2: same industry/family, but allow a cap band already used elsewhere.
            rep = pick(pool)
            if rep is not None:
                reused_band_count += 1
        if rep is None:
    ...
    if reused_band_count:
        notes.append(f"Queue C: {reused_band_count} pick(s) had to reuse a cap band already used by Core/Queue B "
                      f"for that industry — no unused band was available.")

    # Tier 3: still short → Core industries that only qualified in ONE family get a second
    # stock from that SAME family instead (clearly not a true structure contrast, logged).
    if len(rows) < 15:
        placed = {r["Industry"] for r in rows}
        single = core_part[~core_part.Industry.isin(placed)].sort_values("Rate", ascending=False)
        added = 0
        for _, prow in single.iterrows():
            if len(rows) >= 15:
                break
            industry = prow["Industry"]
            core_pick = core[core.Industry == industry].iloc[0]
            pool = hits[(hits.Industry == industry) & (hits.Family == core_pick["Section"])
                        & (~hits.Symbol.isin(used))]
            rep = pick(pool[pool.CapBand != core_pick["Cap Band"]])
            if rep is None:
                rep = pick(pool)
            if rep is None:
                continue
            ...
            rows.append({"Ticker": rep["Symbol"], "Industry": industry, "Sector": rep["Sector"],
                          ...
                          "Cap Band": rep["CapBand"], "Contrasts With": core_pick["Ticker"]})
        if added:
            notes.append(f"Queue C: fewer than 15 Core industries qualified in both families — filled "
                          f"{added} more slot(s) with a same-family second stock instead (not a true "
                          f"structure contrast; flagged here so you can swap these out).")

    if len(rows) < 15:
        notes.append(f"Queue C: only {len(rows)}/15 slots filled even after relaxing the rules — not enough "
                      f"qualifying stock left across the Core industries.")
    return pd.DataFrame(rows)
...
SECTION_META = {
    "core": {"label": "Core 30", "icon": "🟢", "target": 30},
    "queue_a": {"label": "Queue A — Breadth", "icon": "🔵", "target": 15},
    "queue_b": {"label": "Queue B — Cap Contrast", "icon": "🟣", "target": 15},
    "queue_c": {"label": "Queue C — Structure Contrast", "icon": "🟠", "target": 15},
}
...
def main():
    ...
    st.markdown(
        """
        <style>
        .section-card {border: 1px solid rgba(120,120,120,0.25); border-radius: 12px;
                        padding: 1.1rem 1.3rem; margin-bottom: 1.4rem;}
        .section-title {font-size: 1.25rem; font-weight: 700; margin-bottom: 0.1rem;}
        .section-sub {color: rgba(120,120,120,0.9); font-size: 0.88rem; margin-bottom: 0.8rem;}
        </style>
        """,
    ...
    # --- Top summary strip: progress toward the full 75 ---
    total = sum(len(getattr(result, k)) for k in SECTION_META)
    cols = st.columns(5)
    cols[0].metric("Total watchlist", f"{total}/75")
    for i, (key, meta) in enumerate(SECTION_META.items(), start=1):
        n = len(getattr(result, key))
        cols[i].metric(meta["label"], f"{n}/{meta['target']}")

    if result.notes:
        with st.expander("⚠️ Allocation notes (reallocations / shortfalls)", expanded=total < 75):
            for n in result.notes:
                st.write("• " + n)

    st.divider()

    # --- The four core tables, each its own clearly separated, decorated section ---
    SECTION_SUB = {
        "core": "15 TREND + 15 TURN — one stock per industry, largest of the Large cap band.",
        "queue_a": "Best unclaimed industries by participation rate — quadrant quotas 6/5/3/1.",
        "queue_b": "Second stock per Core industry, different cap band (Mid, else Small).",
        "queue_c": "Stock from the family the Core pick did NOT come from.",
    }

    def render_card(key: str):
        meta = SECTION_META[key]
        df = getattr(result, key)
        st.markdown(
            f'<div class="section-card"><div class="section-title">{meta["icon"]} {meta["label"]} '
            f'({len(df)}/{meta["target"]})</div>'
            f'<div class="section-sub">{SECTION_SUB[key]}</div></div>',
            unsafe_allow_html=True,
        )
        if df.empty:
            st.write("No data.")
        ...
        c1, c2 = st.columns(2)
        c1.text_area(f"Paste string — {meta['label']}", paste_string(df), height=100, key=f"paste_{key}")
        c2.text_area(f"Numbered list — {meta['label']}", numbered_list(df), height=100, key=f"list_{key}")
        st.write("")
    ...
    tabs = st.tabs(["Sector Breadth", "Industry Structure", "Participation Ledger", "Reserve List"])
    for tab, df in zip(tabs, [result.sector_breadth, result.industry_structure, result.ledger, result.reserve]):
        with tab:
            if df.empty:
                st.write("No data.")
            else:
